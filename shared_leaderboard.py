...
def _current_scores():
    """
    The bots stay online for a long time, so their local JSON can become
    stale after the other bot awards points. Always prefer the newest
    leaderboard from origin for read-only operations.
    """
    branch = _branch()
